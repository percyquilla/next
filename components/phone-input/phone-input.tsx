...
export function PhoneInput({
  ...
  const variant = variantProp ?? (theme.components?.MuiTextField?.defaultProps?.variant as TextFieldProps['variant']);
  ...
  const renderInput = () => {
    const textFieldProps: TextFieldProps = {
      ...
      fullWidth,
      hiddenLabel: !label,
      placeholder: placeholder ?? 'Ingresa número de teléfono',
      slotProps: {
...
function CustomInput({ ref, ...other }: { ref?: Ref<HTMLInputElement>; [key: string]: unknown }) {
  return <TextField inputRef={ref} {...(other as TextFieldProps)} />;
}

// ----------------------------------------------------------------------

function parseCountryFromPhone(inputValue: string | undefined): CountryCode | undefined {
  const parsed = inputValue ? parsePhoneNumber(inputValue) : undefined;
  return (parsed?.country as CountryCode | undefined) ?? undefined;
}
